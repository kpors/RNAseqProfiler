...
The RNAseqProfiler pipeline automates RNA-seq data processing from raw FASTQ files to quality-checked, mapped, and normalised gene counts, along with visualisation-ready BigWig files.
...
#### Install RNAseqProfiler package
...
```bash
# Go to directory for cloning RNAseqProfiler into
cd /home/username/projects/

# Clone RNAseqProfiler into the directory
git clone https://github.com/kpors/RNAseqProfiler
```
...
### Using the Snakemake pipeline RNAseqProfiler

------

You need to set different parameters and define FASTQ files/SRA Archive IDs before running RNAseqProfiler, this is done in the Sample_sheet.csv file found in the INPUT directory.
...
Here you select if your input data are uploaded to the FASTQ input directory (RNAseqProfiler/input/fastq/) or automatically downloaded from SRA Archive:

| Pick # | Sequencing data input                                      |
| ------ | ---------------------------------------------------------- |
| 1      | Uploaded to input directory (RNAseqProfiler/input/fastq/) |
...
Upload zipped FASTQ files to RNAseqProfiler/input/fastq/
...
Save/export sample sheet as a CSV file named Sample_sheet.csv in /RNAseqProfiler/input/



#### Run the pipeline

You can test if everything is filled out properly using the --dry-run command. First go to the /RNAseqProfiler/ directory and the execute the pipeline.
...
```bash
# Go to RNAseqProfiler directory
cd RNAseqProfiler
